...
// Version: 2.0 - Uses HTTP API with MySQL (not CloudBase native DB)
(function() {
  'use strict';
  ...
  // ============================================
  const API_BASE = 'https://test-3gop834c099077bf-1302819770.ap-shanghai.app.tcloudbase.com/annotations';
  ...
  function loadAnnotations() {
    ...
    console.log('Loading annotations for URI:', window.location.pathname);

    return fetch(`${API_BASE}?uri=${uri}`)
      .then(response => {
        if (!response.ok) {
      ...
      .then(result => {
        // Handle all CloudBase response formats:
        // 1. Direct response (no body property)
        // 2. Wrapper with string body (needs JSON.parse)
        // 3. Wrapper with pre-parsed body object
        let data;
  ...
  function init() {
    ...
    // Load annotations from MySQL via API
    loadAnnotations()
      .then(() => {
